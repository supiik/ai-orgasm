...
|-----------|------|----------|
| `sdk/` | Java 25 library | `proxima-sal-sdk` JAR – consumed by `backend` and `lambda` |
| `backend-core/` | Spring Boot 4 library | `proxima-sal-backend-core` JAR – service layer, JPA, Flyway, datasource config; shared by `backend` and `lambda` |
...
|--------|----------|--------|--------------|----------------|
| `sdk-models/` | `proxima-sal-sdk-models` | Java 11+ | — (models only, shared) | `com.etactics.proxima.sal.sdk.model` |
| `sdk/` | `proxima-sal-sdk` | Java 25 | `java.net.http.HttpClient` (native) | `com.etactics.proxima.sal.sdk.client` |
| `sdk-java11/` | `proxima-sal-sdk-java11` | Java 11+ | `java.net.http.HttpClient` (native) | `com.etactics.proxima.sal.sdk.java11` |
| `sdk-java8/` | `proxima-sal-sdk-java8` | Java 8+ | OkHttp 4 + Gson | `com.etactics.proxima.sal.sdk.java8` |
| `sdk-typescript/` | npm `@etactics/proxima-sal-client` | TypeScript/ESM | Axios | `api/`, `model/` |

**Model consolidation:** `sdk-models` generates one set of Jackson-annotated model classes (`com.etactics.proxima.sal.sdk.model.*`) shared by `sdk/` and `sdk-java11/`. Both modules set `generateModels=false` and point `modelPackage=com.etactics.proxima.sal.sdk.model`; the generator emits API and supporting files only, importing models from `sdk-models.jar`. `sdk-java8` stays independent: its Gson-annotated models (`@SerializedName`) and the `URLEncoder.encode(String, Charset)` Java 10+ API in generated `toUrlQueryString()` helpers make sharing with a `--release 8` target impossible without custom templates.
...
| Name | What it starts |
|------|---------------|
| `Spring Boot-BackendApplication<proxima-sal-backend>` | Spring Boot with `dev` profile, loads `backend/.env` |
...
```bash
# Build native executable to backend/target/proxima-sal-backend
...
docker build -f lambda/Dockerfile -t proxima-sal-lambda:latest .

# For Graviton arm64 (matches template.yaml Architectures: [arm64]):
docker buildx build --platform linux/arm64 \
    -f lambda/Dockerfile -t proxima-sal-lambda:latest .

# Push to ECR
aws ecr get-login-password --region <region> \
  | docker login --username AWS --password-stdin <account>.dkr.ecr.<region>.amazonaws.com
docker tag proxima-sal-lambda:latest <account>.dkr.ecr.<region>.amazonaws.com/proxima-sal-lambda:latest
docker push <account>.dkr.ecr.<region>.amazonaws.com/proxima-sal-lambda:latest

# Deploy via SAM (pass the image URI)
sam deploy --parameter-overrides \
  ImageUri=<account>.dkr.ecr.<region>.amazonaws.com/proxima-sal-lambda:latest \
  Env=dev
```
...
### Lambda packaging
`lambda` uses `maven-shade-plugin` to produce a single fat JAR. The handler class is referenced directly in `template.yaml` (`Handler: com.etactics.proxima.sal.lambda.HelloHandler::handleRequest`). New Lambda functions follow the same pattern: implement `RequestHandler<IN, OUT>`, add a new `AWS::Serverless::Function` resource in `template.yaml`.
...
|----------------|----------|------|---------|
| `appDataSource` / `appEntityManagerFactory` | `proxima_sal` | 3306 | Application data |
| `billingDataSource` / `billingEntityManagerFactory` | `proxima_sal_billing` | 3307 | Billing data |

**Package layout for entities and repositories:**
- `com.etactics.proxima.sal.backend.domain.billing` — `@Entity` classes for the **billing** database
- `com.etactics.proxima.sal.backend.repository.billing` — Spring Data repositories for billing (use `billingTransactionManager`)
- Everything else under `com.etactics.proxima.sal.backend` — entities and repositories for the **app** database

`AppJpaConfig` scans `com.etactics.proxima.sal.backend` broadly for both entities and repositories. `BillingJpaConfig` keeps a narrow scan (`com.etactics.proxima.sal.backend.domain.billing` / `com.etactics.proxima.sal.backend.repository.billing`). This means **new feature packages (e.g. `com.etactics.proxima.sal.backend.song`) are picked up automatically** — no changes to `AppJpaConfig` required when adding a new entity.
...
**Base entity:** All entities should extend `com.etactics.proxima.sal.backend.domain.AuditableEntity` (`@MappedSuperclass`), which provides:
...
class SomeRepositoryIT {
    ...
    static MariaDBContainer<?> appDb = new MariaDBContainer<>("mariadb:12.2.2")
            .withDatabaseName("proxima_sal").withUsername("proxima_sal").withPassword("proxima_sal");
