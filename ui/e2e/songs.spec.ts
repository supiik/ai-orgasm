...
test.describe('songs API (via MSW)', () => {
  ...
  test('lists seed songs', async ({ page }) => {
    const { status, body } = await browserFetch(page, '/api/songs')
  ...
  test('returns 400 when artist is missing', async ({ page }) => {
    const { status } = await browserFetch(page, '/api/songs', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  test('gets a song by id', async ({ page }) => {
    const { status, body } = await browserFetch(page, '/api/songs/1')
    expect(status).toBe(200)
  ...
  test('returns 404 for unknown id', async ({ page }) => {
    const { status } = await browserFetch(page, '/api/songs/9999')
    expect(status).toBe(404)
  })

  test('updates a song', async ({ page }) => {
    const { status, body } = await browserFetch(page, '/api/songs/2', {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
  ...
  test('returns 400 on update when artist is blank', async ({ page }) => {
    const { status } = await browserFetch(page, '/api/songs/1', {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
  ...
  test('deletes a song', async ({ page }) => {
    const del = await browserFetch(page, '/api/songs/3', { method: 'DELETE' })
    expect(del.status).toBe(204)

    const get = await browserFetch(page, '/api/songs/3')
