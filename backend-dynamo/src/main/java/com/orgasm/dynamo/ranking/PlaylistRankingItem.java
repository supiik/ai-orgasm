...
/**
 * DynamoDB item for a PlaylistRanking. Partition key is {@code <tenantId>#PLAYLIST_RANKING} so a
 * per-tenant Query (not a table-wide Scan) can list a tenant's rankings cheaply; sort key is the
 * item id.
 */
@DynamoDbBean
public class PlaylistRankingItem {

    public static String partitionKey(long tenantId) {
        return tenantId + "#PLAYLIST_RANKING";
    }
