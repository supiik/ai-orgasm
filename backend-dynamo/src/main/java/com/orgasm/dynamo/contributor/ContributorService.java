...
public class ContributorService {
    ...
    private final ContributorDynamoRepository repository;
    ...
    public ContributorResponse create(CreateContributorRequest request) {
        ...
        ContributorItem item = mapper.toItem(request, tenantId, id);
    ...
    public Page<ContributorResponse> findAll(FindContributorsRequest request, Pageable pageable) {
        long tenantId = DynamoTenantContext.get();
        List<ContributorItem> items = repository.findAllByTenant(tenantId).stream()
                .filter(item -> item.getDeletedAt() == null)
        ...
        List<ContributorResponse> page = items.subList(from, to).stream().map(mapper::toResponse).toList();
    ...
    public ContributorResponse update(String id, UpdateContributorRequest request) {
        ContributorItem existing = requireById(id);
    ...
    private ContributorItem requireById(String id) {
        long tenantId = DynamoTenantContext.get();
        return repository.findById(tenantId, IdGenerator.parse(id))
                .filter(item -> item.getDeletedAt() == null)
                .orElseThrow(() -> new NoSuchElementException("Contributor not found: " + id));
    }
}
