...
public class ApproveNominationHandler extends BaseHandler<NominationResponse> {

    private final OrgasmService orgasmService;

    public ApproveNominationHandler() {
        var ctx = SpringContextHolder.get();
        this.orgasmService = ctx.getBean(OrgasmService.class);
    }

    ApproveNominationHandler(OrgasmService orgasmService, ObjectMapper mapper, Validator validator) {
    ...
    protected NominationResponse execute(APIGatewayV2HTTPEvent event) throws Exception {
        ...
        return orgasmService.approveNomination(nominationId, request);
    }
}
